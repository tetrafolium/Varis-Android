...
public class SearchHistoryProvider extends SearchRecentSuggestionsProvider {
  public static final String AUTHORITY =
      BuildConfig.APPLICATION_ID + ".storage.SearchHistoryProvider";
  public static final int MODE = DATABASE_MODE_QUERIES;

  // column with the search text
  public static final String HEADER_COLUMN = "display1";

  public SearchHistoryProvider() { setupSuggestions(AUTHORITY, MODE); }
   ...
   */
  public static Cursor queryRecentSearch(final Context context,
                                         final String query) {
    Uri uri = Uri.parse("content://".concat(
        SearchHistoryProvider.AUTHORITY.concat("/suggestions")));

    String[] selection = SearchRecentSuggestions.QUERIES_PROJECTION_1LINE;
    String[] selectionArgs = new String[] {"%" + query + "%"};

    Cursor cursor = context.getContentResolver().query(
        uri, selection, "display1 LIKE ?", selectionArgs, "date DESC");
