 ...
 */
public final class AuthActivity extends MvpActivity<AuthPresenter>
    implements AuthView, AuthFragment.OnLoginActionListener,
               SecurityCodeFragment.OnSecurityCodeAction {

  private static final String AUTH_FRAGMENT_TAG = "AuthFragment";
  private static final String SECURITY_CODE_FRAGMENT_TAG =
  ...
  @Inject PresenterKeeper<MvpPresenter> mPresenterKeeper;
  ...
  private void showLoginSection() {
    AuthFragment authFragment =
        (AuthFragment)getSupportFragmentManager().findFragmentByTag(
            AUTH_FRAGMENT_TAG);
  ...
  private void showSecurityCodeInput() {
    SecurityCodeFragment securityCodeFragment =
        (SecurityCodeFragment)getSupportFragmentManager().findFragmentByTag(
            SECURITY_CODE_FRAGMENT_TAG);
    if (securityCodeFragment == null) {
      securityCodeFragment = SecurityCodeFragment.newInstance();
      replaceFragment(R.id.auth_container, securityCodeFragment,
                      SECURITY_CODE_FRAGMENT_TAG);
    }
  }
  ...
  public void showProgress() {
    mProgressDialog =
        ProgressDialog.show(this, "", getString(R.string.loading_msg));
  }
