 ...
 */
public final class MainActivity extends MvpActivity<RepositoriesPresenter>
	implements RepositoriesView, ReposFragment.ReposFragmentListener {
...
@Inject RepositoriesPresenter mPresenter;
...
protected void onActivityResult(final int requestCode, final int resultCode,
	...
	if (resultCode == RESULT_OK) {
		...
		case REPO_DETAILS_CODE:
			boolean reloadRequired = data.getBooleanExtra(
				RepoDetailsActivity.RELOAD_REQUIRED_KEY, false);
			if (reloadRequired) {
				showProgress();
				getPresenter().reloadRepos();
			}
		...
		mSearchView.setOnQueryTextListener(new SearchView.OnQueryTextListener() {
				...
				public boolean onQueryTextSubmit(final String query) {
				        ...
				        if (query.length() > SEARCH_LIMIT) {
				                ...
				                SearchRecentSuggestions suggestionsProvider =
...
public void updateUserData(final User user) {
	...
	if (user != null) {
		...
		if (!TextUtils.isEmpty(user.getName())) {
			username =
				String.format("%1$s (%2$s)", user.getName(), user.getLogin());
		}
