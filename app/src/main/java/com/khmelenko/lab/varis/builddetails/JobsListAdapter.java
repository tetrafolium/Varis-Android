 ...
 */
public final class JobsListAdapter
    extends RecyclerView.Adapter<JobsListAdapter.JobViewHolder> {

  private final Context mContext;
  private List<Job> mJobs;
  private final OnListItemListener mListener;

  public JobsListAdapter(final Context context, final List<Job> jobs,
  ...
  @Override
  public JobViewHolder onCreateViewHolder(final ViewGroup parent,
                                          final int viewType) {
  ...
  public void onBindViewHolder(final JobViewHolder holder, final int position) {
    if (mJobs != null) {
      ...
      // job data
      holder.mNumber.setText(mContext.getString(
          R.string.build_details_job_number, job.getNumber()));
      ...
      if (!TextUtils.isEmpty(state)) {
        ...
        if (drawable != null) {
          holder.mNumber.setCompoundDrawablesWithIntrinsicBounds(drawable, null,
                                                                 null, null);
        }
      }
      ...
      if (BuildStateHelper.isPassed(job.getState())) {
        ...
        long durationInSeconds =
            (finished.getTime() - started.getTime()) / 1000L;
        String duration = TimeConverter.durationToString(durationInSeconds);
        duration =
            mContext.getString(R.string.build_details_job_duration, duration);
        holder.mDuration.setText(duration);
      } else {
        String stateInProgress =
            mContext.getString(R.string.build_details_job_in_progress);
        String duration = mContext.getString(
            R.string.build_details_job_duration, stateInProgress);
        holder.mDuration.setText(duration);
      }
    }
  }
  ...
  public int getItemHeight() { return JobViewHolder.HEIGHT; }
  ...
  class JobViewHolder
    ...
    @BindView(R.id.card_view) View mParent;

    @BindView(R.id.item_job_number) TextView mNumber;

    @BindView(R.id.item_job_state) TextView mState;

    @BindView(R.id.item_job_duration) TextView mDuration;
