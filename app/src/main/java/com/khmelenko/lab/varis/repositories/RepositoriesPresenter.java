...
@Inject
public RepositoriesPresenter(final TravisRestClient restClient,
	...
	if (StringUtils.isEmpty(accessToken)) {
		...
		Disposable subscription =
			...
			.flatMap(new Function<User, SingleSource<List<Repo> > >() {
				@Override
				public SingleSource<List<Repo> > apply(final @NonNull User user)
				throws Exception {
				        String loginName = mUser.getLogin();
				        return mTravisRestClient.getApiService().getUserRepos(
						loginName);
				}
			})
			.subscribeOn(Schedulers.io())
