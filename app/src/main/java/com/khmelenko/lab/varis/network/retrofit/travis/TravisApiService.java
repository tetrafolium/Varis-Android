...
public interface TravisApiService {
  ...
  // repositories
  @GET("/repos") Single<List<Repo>> getRepos();

  @GET("/repos") Single<List<Repo>> getRepos(@Query("search") String search);

  @GET("/repos/{repositoryId}")
  Single<Repo> getRepo(@Path("repositoryId") long repositoryId);

  @GET("/repos/{repositorySlug}")
  Single<Repo> getRepo(@Path(value = "repositorySlug",
                             encoded = true) String repositorySlug);
  ...
  Single<Branches> getBranches(@Path(value = "repositorySlug",
                                     encoded = true) String repositorySlug);

  @GET("/repos/{repositoryId}/branches/{branch}")
  Single<Branch> getBranch(@Path("repositoryId") long repositoryId,
                           @Path("branch") String branch);

  @GET("/repos/{repositorySlug}/branches/{branch}")
  Single<Branch> getBranch(@Path(value = "repositorySlug",
                                 encoded = true) String repositorySlug,
                           @Path("branch") String branch);
  ...
  Single<BuildHistory> getBuilds(@Path(value = "repositorySlug",
                                       encoded = true) String repositorySlug);

  @GET("/repos/{repositoryId}/builds?event_type=pull_request")
  Single<BuildHistory>
  getPullRequestBuilds(@Path("repositoryId") long repositoryId);

  @GET("/repos/{repositorySlug}/builds?event_type=pull_request")
  Single<BuildHistory> getPullRequestBuilds(
      @Path(value = "repositorySlug", encoded = true) String repositorySlug);

  @GET("/repos/{repositoryId}/builds/{buildId}")
  Single<BuildDetails> getBuild(@Path("repositoryId") long repositoryId,
                                @Path("buildId") long buildId);

  @GET("/repos/{repositorySlug}/builds/{buildId}")
  Single<BuildDetails> getBuild(@Path(value = "repositorySlug",
                                      encoded = true) String repositorySlug,
                                @Path("buildId") long buildId);

  @POST("/builds/{buildId}/cancel")
  Single<Object> cancelBuild(@Path("buildId") long buildId,
                             @Body RequestBody emptyBody);

  @POST("/builds/{buildId}/restart")
  Single<Object> restartBuild(@Path("buildId") long buildId,
                              @Body RequestBody emptyBody);
  ...
  // logs
  @GET("/jobs/{jobId}/log") Single<String> getLogRaw(@Path("jobId") long jobId);

  @GET("/logs/{logId}") Single<Logs> getLogs(@Path("logId") long logId);

  // users
  @GET("/users") Single<User> getUser();
}
