...
public class TestBuildDetailsPresenter {
  ...
  public void setup() {
    ...
    mBuildsDetailsPresenter = spy(
        new BuildsDetailsPresenter(mTravisRestClient, mRawClient, mCacheStorage,
  ...
  public void testStartLoadingLogNoToken() throws Exception {
    ...
    when(mRawClient.getApiService().getLog(String.valueOf(jobId)))
        .thenReturn(Single.just(expectedUrl));
    when(mRawClient.getLogUrl(jobId)).thenReturn(expectedUrl);
    when(mRawClient.singleStringRequest(expectedUrl))
        .thenReturn(Single.just(log));
  ...
  public void testStartLoadingLogWithToken() throws Exception {
    ...
    when(mRawClient.getApiService().getLog(authToken, String.valueOf(jobId)))
  ...
  public void testStartLoadingDataSingleJob() {
    ...
    when(mRawClient.getApiService().getLog(authToken,
                                           String.valueOf(job.getId())))
        .thenReturn(Single.just(""));
    when(mTravisRestClient.getApiService().getBuild(slug, buildId))
  ...
  public void testStartLoadingDataIntentUrl() throws Exception {
    ...
    final String intentUrl =
        "https://travis-ci.org/dkhmelenko/Varis-Android/builds/188971232";

    when(mRawClient.singleRequest(intentUrl))
        .thenReturn(Single.just(intentResponse()));
  ...
  public void testStartLoadingDataIntentUrlFailed() throws Exception {
    ...
    final String slug = "dkhmelenko/Varis-Android";
    final String intentUrl =
        "https://travis-ci.org/dkhmelenko/Varis-Android/builds";

    Response response = intentResponse();
    final String errorMsg = "error";

    when(mRawClient.singleRequest(intentUrl)).thenReturn(Single.just(response));
    when(mTravisRestClient.getApiService().getBuild(any(String.class),
                                                    any(Long.class)))
  ...
  public void testRestartBuild() {
    ...
    when(mTravisRestClient.getApiService().restartBuild(any(Long.class),
  ...
  public void testCancelBuild() {
    ...
    when(mTravisRestClient.getApiService().cancelBuild(any(Long.class),
                                                       any(RequestBody.class)))
        .thenReturn(Single.just(new Object()));
    when(mTravisRestClient.getApiService().getBuild(any(String.class),
                                                    any(Long.class)))
